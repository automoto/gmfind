...
class ProtonDBClient:
    ...
    async def get_rating(self, app_id: int) -> ProtonDBReport:
    ...
    async def get_ratings_batch(self, app_ids: list[int]) -> dict[int, ProtonDBReport]:
        ...
        for app_id in app_ids:
            results[app_id] = await self.get_rating(app_id)
        return results

    def meets_minimum_rating(
        self, report: ProtonDBReport, min_rating: str
    ) -> bool:
